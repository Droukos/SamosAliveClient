...
import {authRSocket} from "@/plugins/api";
...
export default class Validator {
    ...
    async uniqueEmail() {
        ...
        authRSocket.requestResponse({
            ...
            onComplete: value => {
                if (bufToJson(value) == 'false') {
                    this.obj.e = i18n.t("validations.fieldTaken", [i18n.t("fields.email")]);
                }
            }
        })
    }
...
export function validateUniqueUsername(username: FieldObject) {
    if (username.i != "$cancel" && username.i != "$checkOut") {
        ...
        new Validator(username).uniqueUsername().then(() => uniqueProcess(username));
