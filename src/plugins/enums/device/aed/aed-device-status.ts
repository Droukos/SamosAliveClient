...
export function getAedDeviceStatusColor(statusOption: number): string {
  ...
  else if (statusOption == deviceStatus.STOLEN)
    return "red--text text--darken-2";
  else if (statusOption == deviceStatus.ON_REPAIR)
    return "yellow--text text--lighten-1";
  else if (statusOption == deviceStatus.BORROWED)
    return "purple--text text--lighten-1";
  else if (statusOption == deviceStatus.AVAILABLE)
    return "blue--text text--lighten-2";
  else if (statusOption == deviceStatus.ON_RETURN)
